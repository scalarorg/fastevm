...
impl<T: PoolTransaction> CommittedTransactions<T> {
    ...
    pub(crate) fn ancestor(&self, id: &TransactionId) -> Option<&Arc<ValidPoolTransaction<T>>> {
        self.ordered.get(&id.unchecked_ancestor()?)
    }

    // /// Non-blocking read on the new pending transactions subscription channel
    // fn try_recv(&mut self) -> Option<Arc<ValidPoolTransaction<T>>> {
    //     loop {
