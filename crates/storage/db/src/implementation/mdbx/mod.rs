...
/// Parse sync mode from environment variable string.
///
/// Valid values (case-insensitive, dashes and underscores are ignored):
/// - `durable` or `durable-sync` -> `SyncMode::Durable`
/// - `nometasync` or `no-meta-sync` -> `SyncMode::NoMetaSync`
/// - `safenosync` or `safe-no-sync` -> `SyncMode::SafeNoSync`
/// - `utterlynosync` or `utterly-no-sync` -> `SyncMode::UtterlyNoSync`
///
/// Returns `None` if the environment variable is not set or contains an invalid value.
fn parse_sync_mode_from_env() -> Option<SyncMode> {
    env::var("DB_SYNC_MODE").ok().and_then(|value| {
        let normalized = value.trim().to_lowercase().replace(['-', '_'], "");
        match normalized.as_str() {
            "durable" | "durablsync" => Some(SyncMode::Durable),
            "nometasync" => Some(SyncMode::NoMetaSync),
            "safenosync" => Some(SyncMode::SafeNoSync),
            "utterlynosync" => Some(SyncMode::UtterlyNoSync),
            _ => {
                warn!(
                    "Invalid value for DB_SYNC_MODE: '{}'. Valid values are: durable, nometasync, safenosync, utterlynosync. Using default.",
                    value
                );
                None
            }
...
pub struct DatabaseArguments {
    ...
    ///
    /// Available sync modes (ordered from safest to fastest):
    /// - `Durable`: Ensures robust data integrity by writing and flushing metadata to disk after
    ///   each transaction. This is the default and safest option, but may be slower for
    ///   write-intensive workloads.
    /// - `NoMetaSync`: Flushes system buffers to disk once per transaction commit, omitting the
    ///   metadata flush. This can potentially double write performance while maintaining reasonable
    ///   data safety.
    /// - `SafeNoSync`: Disables system buffer flushes during commits but retains previous steady
    ///   commits. Offers a balance between performance and data safety.
    /// - `UtterlyNoSync`: Avoids all sync operations, maximizing performance but risking data
    ///   corruption in case of a crash. Use only when data loss is acceptable.
    ///
    /// The sync mode can be configured via:
    /// 1. Explicitly setting it via `with_sync_mode()` or convenience methods (highest priority)
    /// 2. Setting the `DB_SYNC_MODE` environment variable (read automatically in `new()`)
    /// 3. Defaulting to `SyncMode::Durable` if neither is set
    ///
    /// If [None], the default value (`SyncMode::Durable`) is used.
...
impl DatabaseArguments {
    ...
    ///
    /// The sync mode will be read from the `DB_SYNC_MODE` environment variable if set,
    /// otherwise it defaults to `None` (which will use `SyncMode::Durable` when opening the database).
    pub fn new(client_version: ClientVersion) -> Self {
        Self {
            ...
            exclusive: None,
            sync_mode: parse_sync_mode_from_env(),
            max_readers: None,
        }
    }
    ...
    ///
    /// For better write performance, consider using:
    /// - `SyncMode::NoMetaSync` for a good balance of performance and safety
    /// - `SyncMode::SafeNoSync` for higher performance with some safety guarantees
    /// - `SyncMode::UtterlyNoSync` only if data loss is acceptable (e.g., during testing)
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///     .with_sync_mode(Some(SyncMode::NoMetaSync));
    /// ```
    ...
    /// Set sync mode to `NoMetaSync` for improved write performance.
    ///
    /// This is a convenience method that sets the sync mode to `NoMetaSync`, which can
    /// potentially double write performance while maintaining reasonable data safety.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///
    /// let args = DatabaseArguments::new(ClientVersion::default())
    ///     .with_performance_sync_mode();
    /// ```
    pub const fn with_performance_sync_mode(mut self) -> Self {
        self.sync_mode = Some(SyncMode::NoMetaSync);
        self
    }

    /// Set sync mode to `SafeNoSync` for higher write performance.
    ///
    /// This is a convenience method that sets the sync mode to `SafeNoSync`, which offers
    /// higher performance with some safety guarantees. Use when you need better performance
    /// but still want some protection against data loss.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///     .with_fast_sync_mode();
    /// ```
    pub const fn with_fast_sync_mode(mut self) -> Self {
        self.sync_mode = Some(SyncMode::SafeNoSync);
        self
    }

    /// Set sync mode to `UtterlyNoSync` for maximum write performance.
    ///
    /// **Warning**: This mode risks data corruption in case of a crash. Only use when
    /// data loss is acceptable (e.g., during testing or with external backup mechanisms).
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///
    /// let args = DatabaseArguments::new(ClientVersion::default())
    ///     .with_maximum_performance_sync_mode();
    /// ```
    pub const fn with_maximum_performance_sync_mode(mut self) -> Self {
        self.sync_mode = Some(SyncMode::UtterlyNoSync);
        self
    }

    /// Returns the configured sync mode, or `None` if using the default.
    pub const fn sync_mode(&self) -> Option<SyncMode> {
        self.sync_mode
    }
    ...
    pub fn open(
        ...
        let mode = match kind {
            ...
            DatabaseEnvKind::RW => {
                ...
                inner_env.write_map();
                // Use configured sync mode (from code, environment variable, or default to Durable).
                // The sync mode can be set via:
                // 1. DatabaseArguments::with_sync_mode() or convenience methods (highest priority)
                // 2. DB_SYNC_MODE environment variable (read in DatabaseArguments::new())
                // 3. Default to Durable for maximum data safety
...
mod tests {
    ...
    fn db_cursor_delete_current_non_existent() {
        ...
        assert_eq!(
            cursor.seek_exact(key3),
            Ok(Some((key3, Account::default())))
        );
    }
