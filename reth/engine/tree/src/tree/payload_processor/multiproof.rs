...
/// For large blocks, we use larger chunks to reduce overhead.
const MULTIPROOF_TARGETS_CHUNK_SIZE: usize = 10;
const MULTIPROOF_TARGETS_CHUNK_SIZE_LARGE: usize = 50;
const MULTIPROOF_TARGETS_CHUNK_SIZE_VERY_LARGE: usize = 100;

/// Returns the appropriate chunk size based on total proof targets.
fn get_chunk_size(total_targets: usize) -> usize {
    match total_targets {
        0..=100 => MULTIPROOF_TARGETS_CHUNK_SIZE,
        101..=1000 => MULTIPROOF_TARGETS_CHUNK_SIZE_LARGE,
        _ => MULTIPROOF_TARGETS_CHUNK_SIZE_VERY_LARGE,
    }
}
    ...
    Factory: DatabaseProviderFactory<Provider: BlockReader> + Clone + 'static,
{
    ...
    fn on_prefetch_proof(&mut self, targets: MultiProofTargets) -> u64 {
        ...
        // Use dynamic chunk size based on total targets for better performance on large blocks
        let chunk_size = get_chunk_size(proof_targets.len());
        let mut chunks = 0;
    ...
    fn on_state_update(&mut self, source: StateChangeSource, update: EvmState) -> u64 {
        ...
        // Use dynamic chunk size based on total proof targets for better performance on large blocks
        let total_proof_targets = not_fetched_state_update
            .accounts
            .len()
            .saturating_add(not_fetched_state_update.storages.values().map(|s| s.storage.len()).sum::<usize>());
        let chunk_size = get_chunk_size(total_proof_targets);
        let mut chunks = 0;
