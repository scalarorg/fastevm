    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
            ...
            + 'static,
    {
        ...
        // Use larger cache for large blocks (detected by prefix set size)
        let is_large_block = state_root_config.prefix_sets.account_prefix_set.len() > 1000 ||
            state_root_config.prefix_sets.storage_prefix_sets.values().map(|s| s.len()).sum::<usize>() > 5000;
        let task_ctx = if is_large_block {
            ProofTaskCtx::for_large_block(
        ...
        // Increase concurrency for large blocks to reduce queue time
        let max_proof_task_concurrency = if is_large_block {
            // Double the concurrency for large blocks
            (config.max_proof_task_concurrency() as usize).saturating_mul(2).min(512)
        } else {
            config.max_proof_task_concurrency() as usize
        };
        ...
        // For large blocks, we can use more concurrency since we have more CPU available.
        let max_multi_proof_task_concurrency = if is_large_block {
            max_proof_task_concurrency.saturating_mul(3) / 4  // 75% for large blocks
        } else {
            max_proof_task_concurrency / 2  // 50% for normal blocks
        };
