    ...
    V: EngineValidator<T>,
{
    ...
    fn on_new_payload(
        ...
        self.metrics
            .block_validation
            .record_payload_validation(start.elapsed().as_secs_f64());
    ...
    fn on_new_head(&self, new_head: B256) -> ProviderResult<Option<NewCanonicalChain<N>>> {
        ...
        while current_number > current_canonical_number {
            if let Some(block) = self
                .state
                .tree_state
                .executed_block_by_hash(current_hash)
                .cloned()
            {
    ...
    fn update_latest_block_to_canonical_ancestor(
        ...
        self.state
            .tree_state
            .set_canonical_head(canonical_header.num_hash());
    ...
    fn apply_canonical_ancestor_via_reorg(
        ...
        match self.canonical_block_by_hash(new_head_hash)? {
            Some(executed_block) => {
                ...
                self.canonical_in_memory_state
                    .set_canonical_head(canonical_header.clone());
    ...
    fn is_fork(&self, target: BlockWithParent) -> ProviderResult<bool> {
        ...
        loop {
            ...
            let Some(next_block) = self.sealed_header_by_hash(current_hash)? else {
                break;
            };
    ...
    fn on_forkchoice_updated(
        ...
        if attrs.is_some() {
            self.metrics
                .engine
                .forkchoice_with_attributes_updated_messages
                .increment(1);
        }
        self.canonical_in_memory_state
        ...
        Ok(
            TreeOutcome::new(OnForkChoiceUpdated::valid(PayloadStatus::from_status(
                PayloadStatusEnum::Syncing,
            )))
                ...
                match request {
                    EngineApiRequest::InsertExecutedBlock(block) => {
                            ...
                            == block.recovered_block().parent_hash()
                        {
                            ...
                            self.canonical_in_memory_state
                                .set_pending_block(block.clone());
                        }

                        self.state.tree_state.insert_executed(block.clone());
                        self.metrics
                            .engine
                            .inserted_already_executed_blocks
                            .increment(1);
    ...
    fn on_backfill_sync_finished(
        ...
        // before that
        let Some(backfill_num_hash) =
            self.provider
                .block_hash(backfill_height)?
                .map(|hash| BlockNumHash {
                    hash,
        ...
        if let Ok(Some(new_head)) = self.provider.sealed_header(backfill_height) {
            ...
            self.state
                .tree_state
                .set_canonical_head(new_head.num_hash());
            self.persistence_state
                .finish(new_head.hash(), new_head.number());
        ...
        if let Some(backfill_target) =
            ctrl.block_number()
                .zip(newest_finalized)
                .and_then(|(progress, finalized_number)| {
    ...
    fn on_new_persisted_block(&mut self) -> ProviderResult<()> {
        ...
        self.canonical_in_memory_state
            .remove_persisted_blocks(BlockNumHash {
    ...
    fn on_invalid_new_payload(
        ...
        self.state
            .invalid_headers
            .insert_with_invalid_ancestor(head.hash(), invalid);
    ...
    fn check_invalid_ancestor(&mut self, head: B256) -> ProviderResult<Option<PayloadStatus>> {
        ...
        let Some(header) = self.state.invalid_headers.get(&head) else {
            return Ok(None);
        };
        // populate the latest valid hash field
    ...
    fn backfill_sync_target(
        ...
        if exceeds_backfill_threshold {
            if let Some(state) = sync_target_state {
                ...
                match self
                    .provider
                    .header_by_hash_or_number(state.finalized_block_hash.into())
                {
                    Err(err) => {
    ...
    fn on_canonical_chain_update(&mut self, chain_update: NewCanonicalChain<N>) {
        ...
        self.state
            .tree_state
            .set_canonical_head(chain_update.tip().num_hash());
        ...
        self.canonical_in_memory_state
            .set_canonical_head(tip.clone());

        // Update metrics based on new tip
        self.metrics
            .tree
            .canonical_chain_height
            .set(tip.number() as f64);
    ...
    fn on_disconnected_downloaded_block(
            ...
            self.backfill_sync_target(head.number, missing_parent.number, Some(downloaded_block))
        {
            ...
            return Some(TreeEvent::BackfillAction(BackfillAction::Start(
                target.into(),
            )));
        }
        ...
        Err: From<InsertBlockError<N::Block>>,
    {
        ...
        if self.state.tree_state.canonical_block_hash() == executed.recovered_block().parent_hash()
        {
            debug!(target: "engine::tree", pending=?block_num_hash, "updating pending block");
            self.canonical_in_memory_state
                .set_pending_block(executed.clone());
        }
    ...
    fn compute_trie_input<TP: DBProvider + BlockNumReader>(
        ...
        input.extend_with_blocks(
            blocks
                .iter()
                .rev()
                .map(|block| (block.hashed_state(), block.trie_updates())),
        );
        ...
        match self.find_canonical_header(finalized_block_hash) {
            ...
            Ok(Some(finalized)) => {
                    ...
                    != self.canonical_in_memory_state.get_finalized_num_hash()
                {
                    ...
                    let _ = self
                        .persistence
                        .save_finalized_block_number(finalized.number());
    ...
    fn process_payload_attributes(
        ...
        if let Err(err) = self
            .payload_validator
            .validate_payload_attributes_against_header(&attrs, head)
        {
    ...
    pub(crate) fn remove_before(
        ...
        let num = if let Some(hash) = finalized_hash {
            self.provider
                .block_number(hash)?
                .map(|number| BlockNumHash { number, hash })
        } else {
            None
        };
